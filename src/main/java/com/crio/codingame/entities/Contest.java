...
public class Contest extends BaseEntity{
    ...
    private void validateQuestionList(List<Question> qList, Level contestLevel) throws InvalidContestException {
        ...
        //get level of the firstQuestion
        Level firstQuestionLevel=qList.get(0).getLevel();
         // Check if the level of questions is equal to the contest level
         if (!firstQuestionLevel.equals(contestLevel)) {
            throw new InvalidContestException("Question level must match contest level");
        }
        for(Question quest: qList){
            if(!quest.getLevel().equals(firstQuestionLevel)){
                throw new InvalidContestException("All questions must have the same level");
            }
            questions.add(quest);
        }
